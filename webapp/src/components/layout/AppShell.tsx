...
export function AppShell() {
  const { workspace, workspaceStatus, view } = useAppState();
  ...
  return (
    <div className="flex h-dvh flex-col">
      ...
      <div className="flex min-h-0 flex-1">
        <aside className="hidden w-[260px] shrink-0 overflow-y-auto border-r border-line bg-canvas lg:block">
          <LeftSidebar />
        </aside>
        <main className="flex min-w-0 flex-1 flex-col bg-white">{main}</main>
        {workspace && (
          <aside className="hidden w-[340px] shrink-0 overflow-y-auto border-l border-line bg-canvas xl:block">
            <RightSidebar />
          </aside>
        )}
      </div>

...
description: 검수를 마친 발표팩(07)을 NotebookLM에 넘겨 슬라이드 초안을 만든다(기본은 슬라이드만 — 인포그래픽·브리핑 문서는 사용자가 요청할 때만). `nlm` CLI가 인증돼 있으면 로이드가 직접 돌리고, 아니면 업로드 목록과 프롬프트를 사용자에게 넘긴다. 생성물 검사(Render Gate)와 최종 이관까지 포함. 로이드 전용 — PRESENTATION 모드의 마지막 단계.
user-invocable: true
---
...
## 1. 올릴 것
발표팩 `07_notebooklm_presentation_pack.md`와 핵심 원문(파일·URL)만 올린다. 사용자가 준 참고 디자인은 "이 스타일로"라고 했을 때만 올린다(올리면 겉모습을 베낀다).
...
```
노트북 생성부터 업로드·생성·폴링·`output/` 다운로드·기계 검사·`_nlm_run.md` 기록까지 이어서 간다. 팩의 `# 제작 지시`가 프롬프트가 된다(없으면 멈춘다 — 기본 지시로 대신하지 않는다, E-050).
옵션: `--source <파일>`·`--url <URL>`(반복 가능) · `--artifacts slides,infographic`(요청 시에만) · `--slide-format presenter_slides|detailed_deck` ·
`--slide-length short` · `--timeout 3600` · `--slides-pptx`. `--slide-format`을 생략하면 팩의 덱 유형 파일 §6 값을 쓴다.
...
- **확대해서 볼 것** — 숫자·한정·출처가 있는 장과 모아보기에서 걸린 장만 본다.
  ...
  - 팩에 출처가 있는 장에 출처가 찍혔는가(작은 캡션은 잘 빠진다, E-042) · 한정이 숫자 옆에서 읽히는가
  - 근거 ID·URL·검증 문단 노출
...
목표는 **최종 편집이 쉬운 초안**이다. 선·비율 차이와 공식 외형에 맞는 추가 세부는 통과(최종 편집 목록에 기록), Character Model과 어긋난 외형과 노출된 검증 문단은 불통과다.
